...
export default function MyPage() {
  ...
  const toAbsoluteUrl = (url) => {
    ...
    return `${import.meta.env.VITE_HOST_URL}${url}`;
  };

  const fetchUserInfo = async (id) => {
    const response = await fetch(
      `${import.meta.env.VITE_HOST_URL}/api/club/setting/${id}`,
      {
        method: "GET",
        headers: {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_HOST_URL}/api/club/setting/${clubId}`,
        {
          method: "PUT",
          headers: {
            Authorization: localStorage.getItem("Authorization"),
  ...
  const deleteMem = async () => {
    try {
      const response = await fetch(
        `${import.meta.env.VITE_HOST_URL}/api/club/setting`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json; charset=utf-8",
